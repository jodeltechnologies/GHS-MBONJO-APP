...
export const roles=['principal','vp','teacher','hod','discipline','content_creator','parent'];
export const normalizeMatricule = v => String(v??'').trim().toUpperCase().replace(/[\s‐‑–—]/g,'').replace(/-/g,'');
export const promotionEligible = c => /^Form [1-4][AB]$/.test(c)||c.startsWith('Lower Sixth');
export const studies=(student,subject)=>!Array.isArray(student.subjects)||student.subjects.length===0||student.subjects.includes(subject);
...
export function generateTimetable(assignments,form5End){
 const days=['Monday','Tuesday','Wednesday','Thursday','Friday'],result=[],jobs=[];
 for(const a of assignments){if(!classes.includes(a.class)||!a.teacherId||!a.subject||!Number.isInteger(+a.periods)||+a.periods<1||+a.periods>30)throw Error('Each assignment needs a class, teacher, subject and 1–30 periods.');for(let i=0;i<+a.periods;i++)jobs.push(a);}
...
export function attestationText(d){const verb=d.kind==='assumption'?'assumed':d.kind==='resumption'?'resumed':d.action;if(!['assumed','resumed'].includes(verb))throw Error('Select assumed or resumed.');return `I, the undersigned, ${d.principal}, Principal of Government High School Mbonjo Limbe, certify that ${d.name}, matricule ${d.matricule}, rank ${d.rank}, salary index ${d.salaryIndex}, appointed/transferred by Decision No. ${d.decision}, ${verb} duty on ${d.dutyDate} and is effectively serving as ${d.position} in this institution.\n\nIn testimony whereof, this attestation is issued to serve the purpose for which it is intended.`;}
export function validateRecord(kind,d){
 ...
 if(kind==='mark'){if(!/^Sequence [1-6]$/.test(d.assessment)&&d.assessment!=='Promotion exam')throw Error('Invalid assessment.');if(d.assessment==='Promotion exam'&&!promotionEligible(d.class))throw Error('Form 5 and Upper Sixth do not write promotion exams.');if(!Number.isFinite(+d.mark)||+d.mark<0||+d.mark>20||!Number.isFinite(+d.coefficient)||+d.coefficient<=0)throw Error('Invalid mark or coefficient.');}
 if(kind==='document')attestationText(d);
 return d;
}
