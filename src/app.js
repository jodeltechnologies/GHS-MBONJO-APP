...
function reportHTML(student,marks,assessment,year,peers,stats,subjects=[],subjectStats={}){const m=marks.filter(r=>r.data.studentId===student.id).map(r=>r.data),sum=reportSummary(m);const names=[...new Set([...subjects,...m.map(d=>d.subject)])];const num=n=>n==null?'—':Number(n).toFixed(2);const d=student.data;return `<article class="print-page report">${schoolLetterhead()}<h2>PROGRESS REPORT CARD / BULLETIN DE NOTES</h2><p class="center">ACADEMIC YEAR / ANNÉE SCOLAIRE ${esc(year)} · ${esc(assessment)}</p><div class="report-identity"><div><b>Name / Nom:</b> ${esc(d.name)}<br><b>Date of birth:</b> ${esc(d.birthDate||'—')}<br><b>Place of birth:</b> ${esc(d.birthPlace||'—')}<br><b>Gender:</b> ${esc(d.gender||'—')}<br><b>Parent / guardian contact:</b> ${esc(d.phone||'—')}</div><div><b>Class / Classe:</b> ${esc(d.class)}<br><b>Matricule:</b> ${esc(d.matricule)}<br><b>Enrolment / Effectif:</b> ${stats?.enrolment??'—'}<br><b>Vice Principal:</b> __________________<br><b>SDM:</b> __________________</div>${d.photo?`<img class="student-photo" src="${safeImage(d.photo)}" alt="Student photograph">`:'<div class="student-photo">Photo</div>'}</div><table class="report-marks"><thead><tr><th>Subject / Matière</th><th>Coeff.</th><th>${esc(assessment)}</th><th>Final mark</th><th>Total</th><th>Subject avg.</th><th>Grade</th><th>Rank</th><th>Remarks</th></tr></thead><tbody>${names.map(subject=>{const v=m.find(x=>x.subject===subject),a=list('assignment').find(r=>r.data.class===d.class&&r.data.subject===subject),teacher=a?name(a.data.teacherId):'';return `<tr><td>${esc(subject)}${teacher?`<small>${esc(teacher)}</small>`:''}</td><td>${v?esc(v.coefficient):'—'}</td><td>${v?esc(v.mark):'—'}</td><td>${v?esc(v.mark):'—'}</td><td>${v?num(Number(v.mark)*Number(v.coefficient)):'—'}</td><td>${num(subjectStats[subject]?.average)}</td><td>${esc(v?.grade||'—')}</td><td>${subjectStats[subject]?.rank??'—'}</td><td>${esc(v?.remark||'')}</td></tr>`}).join('')}<tr><th>TOTAL</th><th>${sum.coefficients??m.reduce((n,d)=>n+Number(d.coefficient),0)}</th><td colspan="2"></td><th>${num(sum.total)}</th><td colspan="4"></td></tr></tbody></table><table class="report-summary"><tbody><tr><td>Highest average: ${num(stats?.highest)}</td><td><b>Student average: ${num(sum.average)} / 20</b></td><td>Position: ${stats?.rank??'—'} / ${stats?.rankedCount??0}</td></tr><tr><td>Lowest average: ${num(stats?.lowest)}</td><td>Class average: ${num(stats?.classAverage)}</td><td>${m.length} / ${names.length} subjects assessed</td></tr></tbody></table><div class="report-box"><b>SDM'S REMARKS / OBSERVATIONS DU SURVEILLANT GÉNÉRAL</b><p>Absences: ________ &nbsp; Warnings: ________ &nbsp; Serious warnings: ________</p><p>Punishment: ____________________ Suspension: ____________________</p>Observation: ________________________________________________________</div><div class="report-box"><b>CLASS COUNCIL / CONSEIL DE CLASSE</b><p>Academic warning: __________________ Discipline warning: __________________</p>Observations: ________________________________________________________</div><div class="competency-key">Competency reference: 18–20 A+/E · 15–&lt;18 CA · 11–&lt;15 CECA · 0–&lt;11 CNA</div><div class="report-signatures"><span>Class master / mistress</span><span>Principal / Proviseur</span><span>Parent / Guardian</span></div><small>Provisional until all compulsory and selected subject marks are published. Missing marks are excluded. Letter grades are entered by the teacher under the school's grading policy.</small></article>`;}
...
document.addEventListener('submit',async e=>{e.preventDefault();const form=e.target;if(busy)return;busy=true;const submit=form.querySelector('button[type="submit"],button:not([type])');if(submit)submit.disabled=true;try{const data=Object.fromEntries(new FormData(form));
 ...
 if(form.id==='report-form'){const a=await api('reports',data);if(!a.reports.some(r=>r.marks.length))throw Error('No published marks for this selection.');if(data.output==='master')printView(masterHTML(a.reports,data.assessment,data.year,data.class));else printView(a.reports.map(r=>reportHTML(r.student,r.marks,data.assessment,data.year,[r.student],r.stats,r.subjects,r.subjectStats)).join(''));}
